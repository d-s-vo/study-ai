...
| Redis         | 6379      | контейнер задачи, дефолтный хост-порт ⚠    |
| Mailpit       | 8025 / 1025 | дефолтные хост-порты ⚠                   |

⚠ Redis/Mailpit бронью не изолируются (выделяется только логический `REDIS_DB`, здесь — 5):
контейнеры задачи занимают дефолтные хост-порты. Пока Sail-стек один — конфликта нет;
при втором параллельном стенде первым делом развести эти порты вручную
(`FORWARD_REDIS_PORT`, `FORWARD_MAILPIT_PORT`, `FORWARD_MAILPIT_DASHBOARD_PORT` в .env).
