...
**Обновление 2026-07-31 (поздний вечер): FEAT-010 — 🚧 передано на доставку.** В `feat/recipe-crud` (коммит `632c4aa`, базис develop@`73050e3`) реализованы мутации домена Recipe: RecipePolicy (анти-IDOR, общий каталог), 3 Task, Store/UpdateRecipeRequest, тонкий RecipeController (`findModel`+`Gate::authorize` без упоминания модели), маршруты `recipes.store/update/destroy` + `/profile` под `verified`; арх-барьер allow-list += `App\Policies` (red→green доказан). Гейты: Pest 65/242 (0 fail), PHPStan L10 0 err/0 подавлений, Pint pass; живая приёмка HTTP на стенде worktree (IDOR 403 живьём). Отклонение от спеки: вместо `(int) getAuthIdentifier()` — `$request->user()->id` (Larastan резолвит User из auth-конфига; касты запрещены гейтом). Детали — `devlog/features/FEAT-010-recipe-crud/impl.md`, разбор — `devlog/commits/cbook/632c4aa-recipe-crud.md`. Доставка ветки — за владельцем (после merge: `feat-map.sh set FEAT-010 --status done`, уборка worktree/брони/стенда). **Подхват следующей сессии: FEAT-011 recipe-pages по её спеке** (бронь есть; redirect-таргеты 010 перенаправить на `recipes.show/index`).
