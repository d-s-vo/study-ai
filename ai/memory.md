...
**Обновление 2026-08-03: FEAT-011 recipe-pages — реализована, коммит `6116cff` в `feat/recipe-pages` (базис develop@`632c4aa`), ждёт доставки владельцем (`deliver.sh cbook feat/recipe-pages` → MR в develop).** Первые Page Resolvers (`app/Resolvers/Page/`, модель-free/DTO-only; Show/Edit — чистые трансформеры без DI репозитория — отклонение от спеки, обосновано), read-методы `RecipeController`, GET `recipes.index/create/show/edit` (`create` до `{recipe}`, `whereNumber`), 4 Inertia Vue-страницы (`Pages/Recipe/`, 0 any/as) + пункт меню «Рецепты», пагинация 12/стр (`Paginated<T>` в `resources/js/types/pagination.ts`; рендер через `meta.prev/next_page_url`, без v-html), серверный `canUpdate` на show. Попутно закрыты долги 010: redirect-таргеты (store/update→`recipes.show`, destroy→`recipes.index`) и 3 advisory-нита ревью. `eslint.config.js`: `no-undef` off для ts/vue (ambient `App.*`, рекомендация typescript-eslint). Гейты: Pest 82/369 (0 fail), PHPStan L10 0 err/0 подавлений, Pint, pnpm build (vue-tsc strict) и lint — зелёные; живая приёмка HTTP на стенде брони (порт 8130) — PASS все сценарии (каталог/пагинация/canUpdate/403/404/полный цикл форм), сериализация Spatie `PaginatedDataCollection` подтверждена живьём. Ревью: ADR-010 **PASS** (approve-with-nits), ревью чистоты **ЧИСТО**. Клиентские README/CLAUDE.md — needs propagation (формулировка меню — согласовать с владельцем). После merge: уборка (стенд `down -v`, worktree, бронь), feat-map → done, и первый запуск **process-retro M7** по закрытию пакета FEAT-009…011. Детали — `devlog/features/FEAT-011-recipe-pages/impl.md`; разбор/ревью — `devlog/commits/cbook/6116cff-*`.
