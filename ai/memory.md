...
**Обновление 2026-07-31 (вечер), срез develop@`18c7282` (FEAT-005…008 влиты):** стартовал **доменный пакет Recipe/Ingredient** — брони FEAT-009/010/011 (`recipe-data-layer`/`recipe-crud`/`recipe-pages`), спеки всех трёх готовы (specs-first) с решениями владельца: `recipes.user_id`+`RecipePolicy` (поправка ТЗ, анти-IDOR), модель просмотра — **общий каталог** (index/show всем auth+verified, мутации — владелец), `/profile` — под `verified` (включено в спеку FEAT-010). **FEAT-009 — 🚧 передана владельцу на доставку:** ветка `feat/recipe-data-layer`, коммиты `c48543f`+`73050e3`, оба PASS ревью (ADR-010) + ревью чистоты; гейты Pest 51/PHPStan L10 0 err/Pint OK; изолированный стенд `cbook_recipedata` поднят (тирдаун — после доставки). **Подхват для следующей сессии: после merge FEAT-009 в develop → `task.sh new recipe-crud` → реализация FEAT-010 по её спеке** (worktree НЕ создан намеренно — ветка должна пойти от develop с влитым 009); затем FEAT-011; при закрытии пакета — первый запуск process-retro (M7) + Шаг 10 по каждой фиче (см. gotcha о реестрах). В `coord.sh book` добавлена кросс-валидация реестров FEAT против факта (папок devlog) после инцидента выдачи занятых номеров.
